...
# 시작 메세지 슬랙 전송
post_message(myToken,"#자동투자_ai_이익손절", "autotrade start")
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            if target_price < current_price and current_price < predicted_close_price and ma15 < current_price:
                ...
                if krw > 5000:
                    ...
                    post_message(myToken,"#자동투자_ai_이익손절", "BTC buy : " +str(buy_result))
            if predicted_close_price < target_price and target_price < current_price:
                sell_result = upbit.sell_market_order("KRW-BTC", btc*0.9995)
                post_message(myToken,"#자동투자_ai_이익손절", "BTC buy : " +str(sell_result))
        else:
            btc = get_balance("BTC")
            if btc > 0.0001:
                sell_result = upbit.sell_market_order("KRW-BTC", btc*0.9995)
        ...
        print(e)
        post_message(myToken,"#자동투자_ai_이익손절", e)
        time.sleep(1)