...
# 시작 메세지 슬랙 전송
post_message(myToken,"#자동투자", "autotrade start")
        ...
        if start_time < now < end_time - datetime.timedelta(minutes=1):
            ...
            if target_price < current_price and ma15 < current_price:
                ...
                if krw > 5000:
                    ...
                    post_message(myToken,"#자동투자", "BTC buy : " +str(buy_result))
        else:
            btc = get_balance("BTC")
            if btc > 0.0001:
                sell_result = upbit.sell_market_order("KRW-BTC", btc*0.9995)
                post_message(myToken,"#자동투자", "BTC buy : " + str(sell_result))

        time.sleep(1)
    except Exception as e:
        print(e)
        post_message(myToken,"#자동투자", e)
        time.sleep(1)